...
import { Phone, Lock, Mail, Film, Gift, Facebook, Instagram, Twitter, Youtube, Music } from 'lucide-react';
...
const StandoutAccounts = () => {
  ...
  const resolveIcon = (item: any) => {
    ...
    if (title.includes('facebook')) return { Icon: Facebook, colorClass: 'text-facebook' };
    if (title.includes('instagram')) return { Icon: Instagram, colorClass: 'text-instagram' };
    if (title.includes('twitter') || title.includes('x ')) return { Icon: Twitter, colorClass: 'text-twitter' };
    if (title.includes('tiktok')) return { Icon: Music, colorClass: 'text-tiktok' };
    if (title.includes('youtube') || title.includes('netflix')) return { Icon: Youtube, colorClass: 'text-youtube' };
    ...
    <div className="overflow-x-auto rounded-2xl shadow-xl border border-gray-200">
      <table className="w-full min-w-[800px] bg-white">
        ...
        <tbody className="divide-y divide-gray-200">
          {[
            { title: 'USA WhatsApp number', desc: "USA WhatsApp number for sale, it's a one time verification number. Code will be available after purchase", vendor: 'AS Digitals', delivery: 'Delivers in: 5 mins', price: '$4.49', Icon: Phone },
            { title: 'USA WhatsApp number', desc: "USA WhatsApp number for sale, it's a one time verification number. Code will be available after purchase", vendor: 'AS Digitals', delivery: 'Delivers in: 5 mins', price: '$4.49', Icon: Phone },
            { title: 'Facebook Aged Account 2015', desc: 'Old Facebook account with Marketplace & Ads access. No restrictions. Clean history.', vendor: 'SocialVault', delivery: 'Delivers in: 5 mins', price: '$45.00', Icon: Facebook },
            { title: '1 year Active Pia VPN', desc: '1 year pia vpn works with either ios/android, just 1 device.', vendor: 'lavidamide Logs', delivery: 'Delivers Instantly', price: '$2.98', Icon: Lock },
            { title: 'Gmail PVA Aged 2020', desc: 'High-quality account • Instant delivery • Full warranty', vendor: 'MailKing', delivery: 'Delivers in: 3 mins', price: '$1.99', Icon: Mail },
            { title: 'Netflix Premium 4K', desc: 'High-quality account • Instant delivery • Full warranty', vendor: 'StreamZone', delivery: 'Delivers Instantly', price: '$7.99', Icon: Film },
            { title: '$50 Amazon Gift Card US', desc: 'High-quality account • Instant delivery • Full warranty', vendor: 'GiftZone', delivery: 'Delivers Instantly', price: '$46.50', Icon: Gift },
          ].map((item, idx) => (
            <tr
              key={idx}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="text-3xl">
                        <IconComp className={`w-7 h-7 ${resolved.colorClass}`} />
                      </div>
                    );
                  })()}
        ...
        <div key={idx} className="bg-white border border-gray-200 rounded-xl p-5 shadow-md">
          <div className="flex justify-between items-start mb-3">
              ...
              {(() => {
                const resolved = resolveIcon(item);
                const IconComp = resolved.Icon;
                return <IconComp className={`w-7 h-7 ${resolved.colorClass}`} />;
              })()}
