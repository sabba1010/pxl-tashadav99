...
const MyReport: React.FC = () => {
    ...
    const { data: reports = [] as Report[], isLoading, refetch } = useQuery<Report[]>({
        ...
        queryFn: async () => {
            ...
            const res = await axios.get<Report[]>(`http://localhost:5000/my-reports?email=${user?.email}`);
            return res.data;
        },
        enabled: !!user?.email 
    });
    ...
    const handleMarkSold = async (id: string) => {
        try {
            const res = await axios.patch<any>(`http://localhost:5000/report/mark-sold/${id}`);
            if (res.data.success) {
