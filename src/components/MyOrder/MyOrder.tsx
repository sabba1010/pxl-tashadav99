  ...
  useEffect(() => {
    const checkUnread = async () => {
      ...
      await Promise.all(orders.map(async (p) => {
        ...
        try {
          const res = await axios.get(`${CHAT_API}/history/${sellerId}/${p.buyerEmail}`, {
            params: { orderId: p.id }
          });
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Manage your sold items</p>
            </div>
          </div>
          ...
          <div className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:max-w-lg w-full bg-white rounded-t-3xl sm:rounded-3xl z-50 max-h-[90vh] sm:max-h-[80vh] overflow-y-auto shadow-2xl animate-in fade-in zoom-in duration-300">
            <div className="sticky top-0 bg-white border-b sm:border-b-0 px-6 py-4 flex justify-between items-center z-10">
              <div>
                ...
                <div className="text-xs text-gray-500 mt-0.5">{selected.platform} • Sold on {selected.date}</div>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 text-sm">
                <div><p className="text-gray-500">Order Number</p><p className="font-medium">{selected.orderNumber || "—"}</p></div>
                <div><p className="text-gray-500">Buyer</p><p className="font-medium text-blue-600">{getBuyerDisplayName(selected.buyerEmail)}</p></div>
              ...
              <div className="mt-6 flex gap-2">
                <button
                  onClick={() => { setSelected(null); handleOpenChat(selected); }}
        ...
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:max-w-md w-full bg-[#ECE5DD] sm:rounded-2xl z-[110] shadow-2xl flex flex-col h-[85vh] sm:h-[600px] overflow-hidden animate-in slide-in-from-bottom-10 duration-300 border border-gray-200">
            ...
            <div className="bg-white/95 backdrop-blur-md px-4 py-3 border-b flex justify-between items-center shadow-sm z-20">
              ...
              <button onClick={() => setIsChatOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-red-50 hover:text-red-500 transition-colors">
                <FaTimesIcon size={14} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ backgroundImage: "radial-gradient(#cbd5e1 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              {chatMessages.map((m, i) => {
                const isMe = m.senderId.toLowerCase() === sellerId?.toLowerCase();
            ...
            <form onSubmit={sendChat} className="p-3 bg-white border-t flex gap-2 items-center z-20">
               ...
               <button type="submit" className="bg-[#0A1A3A] text-white p-2.5 rounded-full hover:bg-black transition-colors">
