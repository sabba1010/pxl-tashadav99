...
const Marketplace: React.FC = () => {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Collect ALL selected subcategories into a Set (independent of main category)
    const selectedSubs = new Set<string>();
    Object.values(selectedSubcats).forEach((subs) => {
      subs.forEach((sub) => selectedSubs.add(sub));
    });
    ...
    return items.filter((item) => {
      ...
      // Category filter: if no subcats selected → show all
      // otherwise → item.subcategory must be in selectedSubs
      const matchesCategory =
        selectedSubs.size === 0 ||
        selectedSubs.has(item.subcategory || "");
