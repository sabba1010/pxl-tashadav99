          ...
          <div className="flex items-center justify-between h-14 md:h-16">
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="relative" ref={notifRef}>
                <button onClick={handleShowNotifications} className="p-2 rounded-md hover:bg-gray-100 transition relative">
                  <svg className="w-6 h-6" fill="none" stroke={EMPIRE_BLUE} viewBox="0 0 24 24">
                ...
                {notifOpen && (
                  <div className="fixed top-14 left-4 right-4 md:absolute md:top-full md:left-auto md:right-0 md:mt-2 md:w-96 bg-white border rounded-xl shadow-2xl z-50 overflow-hidden" style={{ backgroundColor: CLEAN_WHITE }}>
                    ...
                    <div className="px-4 py-3 border-t bg-gray-50 flex justify-between items-center">
                      <button onClick={clearAllNotifications} className="text-xs text-red-600 hover:text-red-800 hover:underline font-bold flex items-center gap-1" disabled={notifications.length === 0}>
                        <FaTrashIcon /> Clear All
                      </button>
                      <button onClick={() => fetchNotifications()} className="text-xs text-blue-700 hover:underline font-bold">Refresh List</button>
                    </div>
                  </div>
                )}
              </div>
                ...
                {open && (
                  <div className="fixed left-4 right-4 top-20 md:absolute md:right-0 md:top-full md:left-auto md:mt-2 md:w-80 rounded-xl shadow-2xl border border-gray-200 overflow-hidden z-50 max-h-[70vh] flex flex-col" style={{ backgroundColor: CLEAN_WHITE }}>
                    ...
                    <div className="overflow-y-auto flex-1 py-2">
                      ...
                      {(loginUser?.role === "seller") && (
                        <NavLink to="/seller-chat" onClick={() => setOpen(false)} className="flex items-center gap-3 px-4 md:px-6 py-2.5 md:py-3 hover:bg-gray-50 transition"><span className="text-xs md:text-sm font-medium">Seller Chat</span></NavLink>
                      )}
                    </div>
