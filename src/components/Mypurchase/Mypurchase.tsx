...
const MyPurchase: React.FC = () => {
  ...
  useEffect(() => { chatLengthRef.current = 0; }, [isChatOpen, activeChatOrderId]);
  ...
  return (
    <>
      <div className="min-h-screen bg-[#F3EFEE] pt-16 pb-20">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
            <div><h1 className="text-xl sm:text-3xl font-bold text-[#0A1A3A]">My Purchase</h1></div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-4 sm:px-6 pt-4 sm:pt-6">
              <nav className="flex gap-4 sm:gap-6 border-b border-gray-100 pb-3 sm:pb-4 overflow-x-auto">
                {TABS.map((t) => (
                  <button key={t} onClick={() => setActiveTab(t)} className={`pb-2 text-xs sm:text-sm whitespace-nowrap transition-colors ${activeTab === t ? "text-[#d4a643] border-b-2 border-[#d4a643]" : "text-gray-500"}`}>{t}</button>
                ))}
              </nav>
            </div>
              ...
              <div className="space-y-3">
                {isLoading ? (
                  ...
                  <div className="py-20 flex flex-col items-center text-center">
                    ...
                    <Link to="/marketplace" className="mt-4 bg-[#33ac6f] text-white px-6 py-2 rounded-full text-sm font-medium">Browse Marketplace</Link>
                  </div>
                ) : (
                  filtered.map((p) => (
                    <div key={p.id} className="bg-[#F8FAFB] rounded-xl p-3 sm:p-4 flex items-start gap-3 border border-gray-100 hover:shadow-md transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${p.status === "Completed" ? "bg-green-50 text-green-700 border-green-100" : p.status === "Pending" ? "bg-amber-50 text-amber-700 border-amber-100" : "bg-red-50 text-red-700 border-red-100"}`}>{p.status}</span>
                      ...
                      <div className="flex-shrink-0 text-right flex flex-col items-end gap-2">
                        ...
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => handleOpenChat(p)} className="relative p-1.5 border rounded bg-white hover:bg-blue-50 text-blue-600 transition-colors">
      ...
      {selected && (
        <>
          <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm transition-opacity" onClick={() => setSelected(null)}/>
          <div className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:max-w-lg w-full bg-white rounded-t-3xl sm:rounded-3xl z-50 max-h-[90vh] sm:max-h-[80vh] overflow-y-auto shadow-2xl animate-in fade-in zoom-in duration-300">
             <div className="sticky top-0 bg-white border-b sm:border-b-0 px-6 py-4 flex justify-between items-center z-10">
              ...
              <button onClick={() => setSelected(null)} className="p-2 rounded-full hover:bg-gray-100 transition"><FaTimesIcon className="w-5 h-5 text-gray-600" /></button>
            </div>
             <div className="p-6">
                <div className="flex justify-center">{renderBadge(selected.platform, 72)}</div>
                <div className="mt-4 text-3xl font-bold text-[#0A1A3A] text-center sm:text-left">${selected.price.toFixed(2)}</div>
                <div className="mt-4 grid grid-cols-1 gap-3 text-sm">
                  <div><p className="text-gray-500">Purchase Number</p><p className="font-medium text-gray-900">{selected.purchaseNumber || "—"}</p></div>
                ...
                <div className="mt-6 flex gap-2">
                  ...
                  <button onClick={() => setSelected(null)} className="flex-1 py-2 border rounded hover:bg-gray-50">Close</button>
                </div>
             </div>
          </div>
        </>
      ...
      {isChatOpen && activeChatSellerEmail && (
        <>
          <div className="fixed inset-0 bg-slate-900/70 z-[100] backdrop-blur-sm transition-opacity" onClick={() => setIsChatOpen(false)}/>
          <div className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:max-w-md w-full bg-[#ECE5DD] sm:rounded-2xl z-[110] shadow-2xl flex flex-col h-[85vh] sm:h-[600px] overflow-hidden animate-in slide-in-from-bottom-10 duration-300 border border-gray-200">
            ...
            <div className="bg-white/95 backdrop-blur-md px-4 py-3 border-b flex justify-between items-center shadow-sm z-20">
              ...
              <button onClick={() => setIsChatOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-red-50 hover:text-red-500 transition-colors"><FaTimesIcon size={14} /></button>
            </div>
              ...
              {chatMessages.map((m, i) => {
                ...
                return (
                    <div key={i} className={`flex w-full ${isMe ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[75%] px-4 py-2 text-sm shadow-sm relative group ${isMe ? "bg-gradient-to-r from-[#33ac6f] to-[#2d9660] text-white rounded-2xl rounded-tr-sm" : "bg-white text-gray-800 rounded-2xl rounded-tl-sm border border-gray-100"}`}>
                        {m.message}
                        {timeLabel && <div className={`text-[9px] mt-1 opacity-70 flex justify-end ${isMe ? "text-green-100" : "text-gray-400"}`}>{timeLabel}</div>}
                        </div>
                    </div>
                );
              })}
            ...
            {showEmojiPicker && (
                <div className="bg-white border rounded-xl shadow-lg p-2 absolute bottom-20 left-4 z-30 w-64 grid grid-cols-7 gap-1">
                    {EMOJI_LIST.map(emoji => (
                        <button key={emoji} onClick={() => handleAddEmoji(emoji)} className="p-1 hover:bg-gray-100 rounded text-lg">{emoji}</button>
                    ))}
                </div>
            )}
            ...
            <form onSubmit={sendChat} className="p-3 bg-white border-t flex gap-2 items-center z-20">
              ...
              <button type="submit" disabled={!typedMessage.trim()} className={`p-3 rounded-full shadow-lg transition-all transform active:scale-95 flex items-center justify-center ${typedMessage.trim() ? "bg-[#33ac6f] text-white hover:bg-[#2aa46a] shadow-green-500/30" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}><FaPaperPlaneIcon className="ml-0.5" size={14} /></button>
            </form>
          </div>
        </>
      )}
      <Link to="/add-product" className="hidden sm:flex fixed bottom-6 right-6 w-14 h-14 bg-[#33ac6f] text-white rounded-full shadow-2xl items-center justify-center text-xl hover:scale-110 transition-transform hover:shadow-green-500/50 z-30"><FaPlusIcon /></Link>
    </>
  );
};
