...
const MyPurchase: React.FC = () => {
  ...
  const fetchChat = async (sId: string) => {
    try {
      const res = await axios.get<IMessage[]>(`${CHAT_API}/history/${buyerId}/${sId}`);
  ...
  return (
    <>
              ...
              <div className="space-y-3">
                {filtered.length === 0 ? (
                  <div className="py-12 sm:py-20 flex flex-col items-center text-center text-gray-500">
                    ...
                    <Link to="/marketplace" className="mt-4 sm:mt-6 bg-[#33ac6f] text-[#111111] px-5 py-2 rounded-full font-medium">
                      Browse Marketplace
                    </Link>
                  </div>
                ) : (
                    ...
                    >
                      ...
                      <div className="flex-shrink-0 w-24 text-right flex flex-col items-end gap-2">
                        ...
                        <div className="mt-1 flex gap-1">
                          <button onClick={() => setSelected(p)} className="px-2 py-1 border rounded text-xs bg-white">
                            <FaEyeIcon className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => { setActiveChatSeller(p.seller); setIsChatOpen(true); }}
        ...
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:max-w-md w-full bg-white rounded-t-3xl sm:rounded-2xl z-[110] shadow-2xl flex flex-col h-[80vh] sm:h-[550px] overflow-hidden">
            <div className="bg-[#0A1A3A] p-4 text-white flex justify-between items-center">
              ...
              <button onClick={() => setIsChatOpen(false)}><FaTimesIcon size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
              {chatMessages.map((m, i) => (
                <div key={i} className={`flex ${m.senderId === buyerId ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${
                    m.senderId === buyerId ? "bg-blue-600 text-white rounded-tr-none" : "bg-white border text-gray-800 rounded-tl-none shadow-sm"
                  }`}>
                    {m.message}
                  </div>
                </div>
            ...
            <form onSubmit={sendChat} className="p-4 border-t bg-white flex gap-2">
              <input
                value={typedMessage}
              ...
              />
              <button type="submit" className="bg-[#33ac6f] text-white px-5 py-2 rounded-full text-sm font-bold">
                Send
              </button>
            </form>
          </div>
          ...
          <div className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:max-w-lg bg-white rounded-t-3xl sm:rounded-3xl z-50 max-h-[90vh] sm:max-h-[80vh] overflow-y-auto shadow-2xl">
            ...
            <div className="sticky top-0 bg-white border-b sm:border-b-0 px-6 py-4 flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <div className={`text-xs font-semibold px-2 py-1 rounded ${
                  ...
                  selected.status === "Pending" ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"
                }`}>
                  {selected.status}
                </div>
                <button onClick={() => setSelected(null)} className="p-2 rounded-md hover:bg-gray-100"><FaTimesIcon className="w-5 h-5" /></button>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 text-sm">
                <div><p className="text-gray-500">Purchase Number</p><p className="font-medium">{selected.purchaseNumber ?? "—"}</p></div>
                <div><p className="text-gray-500">Seller</p><p className="font-medium">{selected.seller}</p></div>
              ...
              <div className="mt-4 border p-3 rounded-md bg-[#FBFFFB]">
                <div className="flex items-center justify-between">
                  <div><div className="text-sm font-medium">Buyer Protection</div><div className="text-xs text-gray-600">Secure transaction & refund policy</div></div>
              ...
              <div className="mt-6 grid grid-cols-1 gap-3">
                {selected.status === "Pending" && (
                  <button onClick={handleConfirmOrder} className="w-full py-2 bg-[#33ac6f] hover:bg-[#2aa46a] text-[#111111] rounded font-medium transition shadow-sm">
                    Confirm Order
                  </button>
                )}
                <div className="flex gap-2">
                  <button
                    onClick={() => { setSelected(null); setActiveChatSeller(selected.seller); setIsChatOpen(true); }}
      ...
      <Link to="/add-product" className="hidden sm:flex sm:fixed bottom-6 right-6 w-14 h-14 bg-[#33ac6f] hover:bg-[#c4963a] text-white rounded-full shadow-2xl items-center justify-center text-2xl">
        <FaPlusIcon />
      </Link>
    </>
  );
