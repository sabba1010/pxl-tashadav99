...
const MyPurchase: React.FC = () => {
 ...
 const getRemainingTime = (rawDate: string) => {
  ...
  useEffect(() => {
    let timer: any;
    if (isChatOpen) {
      // mark buyer online for chat
      setPresence('online');
      fetchChat();
      fetchSellerStatus();
      timer = setInterval(() => { fetchChat(); fetchSellerStatus(); }, 3000);
    } else {
      // when chat closed, mark offline
      setPresence('offline');
  ...
  // Poll for order-specific unread chat notifications for this buyer
  ...
  // When chat opens, mark order notifications read
  useEffect(() => {
  ...
  // ensure we set offline on page unload
  useEffect(() => {
    const onUnload = () => {
      try {
        if (!buyerId) return;
        const payload = JSON.stringify({ userId: buyerId, status: 'offline' });
        if (typeof navigator !== 'undefined' && 'sendBeacon' in navigator && (navigator as any).sendBeacon) {
          try {
            (navigator as any).sendBeacon(`${CHAT_API}/status`, payload);
            return;
          } catch (e) {
        ...
        // fallback
        fetch(`${CHAT_API}/status`, { method: 'POST', body: payload, headers: { 'Content-Type': 'application/json' }, keepalive: true });
      } catch (e) {}
    };
