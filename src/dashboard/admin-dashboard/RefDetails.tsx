...
const RefDetails = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get<UserType[]>("https://vps-backend-server-beta.vercel.app/api/user/getall");
  ...
  const updateStatus = async (userId: string, status: ReferralStatus) => {
    ...
    try {
      await axios.patch("https://vps-backend-server-beta.vercel.app/api/user/admin/update-referral-status", {
        userId,
        status,
