...
const TotalListings: React.FC = () => {
  ...
  const fetchListingsData = async () => {
    const [listingsRes, usersRes] = await Promise.all([
      fetch("http://72.244.153.24:3200/product/all-sells"),
      fetch("http://72.244.153.24:3200/api/user/getall"),
    ]);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      const res = await fetch(`http://72.244.153.24:3200/product/update-status/${selected._id}`, {
        method: "PATCH",
