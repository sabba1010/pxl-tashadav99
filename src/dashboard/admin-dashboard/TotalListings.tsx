...
const TotalListings: React.FC = () => {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      const response = await fetch(`https://vps-backend-server-beta.vercel.app/product/update-status/${selected._id}`, {
        method: 'PATCH',
  ...
  return (
    <Box sx={{ p: 4, bgcolor: "#F9FAFB", minHeight: "100vh" }}>
      ...
      <Paper elevation={0} sx={{ p: 4, mb: 4, borderRadius: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #F2F2F2' }}>
        <Typography variant="h5" fontWeight="700">Listings ({filtered.length})</Typography>
        <Box sx={{ position: 'relative', width: 320 }}>
          <Search sx={{ position: 'absolute', left: 15, top: '50%', transform: 'translateY(-50%)', color: '#9CA3AF' }} />
          <InputBase 
            placeholder="Search..." 
            value={search}
            onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            sx={{ bgcolor: '#fff', border: '1px solid #D1D5DB', borderRadius: '10px', pl: 6, pr: 2, py: 1, width: '100%' }}
          />
        </Box>
      </Paper>

      {/* Table */}
      <TableContainer component={Paper} elevation={0} sx={{ borderRadius: 4, border: '1px solid #F2F2F2' }}>
        <Table>
          <TableHead sx={{ bgcolor: "#F9FAFB" }}>
            <TableRow>
              {["ID", "Title", "Seller", "Price", "Status", "Actions"].map((head) => (
            ...
            {paginated.map((row) => {
              ...
              return (
                <TableRow key={row._id} hover>
                  ...
                  <TableCell>
                    <IconButton size="small" color="success" onClick={() => { setSelected(row); setOpenView(true); }}><Visibility fontSize="small" /></IconButton>
                    <IconButton size="small" color="warning" onClick={() => { setSelected(row); setNewStatus(row.status); setOpenEdit(true); }}><Edit fontSize="small" /></IconButton>
                  </TableCell>
                </TableRow>
              );
            })}
      ...
      <Dialog open={openView} onClose={() => setOpenView(false)} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle sx={{ fontWeight: 700, display: 'flex', justifyContent: 'space-between', bgcolor: '#F9FAFB' }}>
          Details <IconButton onClick={() => setOpenView(false)} size="small"><Close /></IconButton>
        </DialogTitle>
        <DialogContent dividers>
          {selected && (
            <Box sx={{ p: 1 }}>
              <Typography variant="h6" fontWeight="600">{selected.name}</Typography>
              <Typography color="text.secondary" sx={{ mb: 2 }}>{selected.category}</Typography>
              <Paper variant="outlined" sx={{ p: 2, bgcolor: '#F8F9FA', mb: 2 }}>
                <Typography variant="body2"><strong>Login Email:</strong> {selected.email}</Typography>
                <Typography variant="body2"><strong>Password:</strong> {selected.password}</Typography>
              </Paper>
              <Typography variant="body2"><strong>Seller:</strong> {selected.userEmail}</Typography>
              <Typography variant="body2" sx={{ mt: 1 }}><strong>Description:</strong> {selected.description}</Typography>
            </Box>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={openEdit} onClose={() => setOpenEdit(false)} PaperProps={{ sx: { borderRadius: 3, width: '100%', maxWidth: '380px' } }}>
        ...
        <DialogContent>
          ...
          <Button 
            ...
            sx={{ mt: 3, bgcolor: "#33ac6f", borderRadius: 2, py: 1.5, fontWeight: 600, '&:hover': { bgcolor: '#2a8e5b' } }}
          >
            Confirm Change
          </Button>
        </DialogContent>
