...
const RatingsReputationPanel: React.FC = () => {
  ...
  const fetchAdminData = async () => {
    setLoading(true);
    try {
      const response = await axios.get<{ success: boolean, data: any[] }>("https://acctempire.com/api/reputation/summary");
  ...
  const handleDeleteReview = async (ratingId: string) => {
    if (window.confirm("Are you sure you want to delete this review?")) {
      try {
        await axios.delete(`https://acctempire.com/api/rating/delete/${ratingId}`);
