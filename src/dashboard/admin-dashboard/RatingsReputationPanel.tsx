...
const RatingsReputationPanel: React.FC = () => {
  ...
  const fetchAdminData = async () => {
    setLoading(true);
    try {
      const response = await axios.get<{ success: boolean, data: any[] }>("http://72.244.153.24:3200/reputation/summary");
  ...
  const handleDeleteReview = async (ratingId: string) => {
    if (window.confirm("Are you sure you want to delete this review?")) {
      try {
        await axios.delete(`http://72.244.153.24:3200/rating/delete/${ratingId}`);
