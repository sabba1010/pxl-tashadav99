...
const WithdrawalModal: React.FC<WithdrawalModalProps> = ({
  ...
  return (
    <Dialog open={!!request} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent dividers>
        {request.hasUnfinishedOrders && (
          <Box sx={{ mb: 2, p: 2, bgcolor: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 1 }}>
            <Typography sx={{ fontWeight: 700, color: '#92400E' }}>Withdrawal blocked</Typography>
            <Typography sx={{ fontSize: '0.9rem', color: '#92400E' }}>Seller has unfinished orders or disputes — payouts are blocked until orders are resolved.</Typography>
          </Box>
        )}
        <Box sx={{ mb: 2, textAlign: "center" }}>
          <Typography variant="caption" sx={{ color: "#6B46C1", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>
      ...
      <DialogActions sx={{ p: 3 }}>
        ...
        <Button
          ...
          disabled={
            ...
            (selectedStatus === "approved" && request.hasUnfinishedOrders)
          }
          onClick={() => onUpdateStatus(request._id, selectedStatus, declineReason)}
          sx={{ backgroundColor: "#6B21B6", '&:hover': { backgroundColor: '#581C87' } }}
        >
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        try {
          ...
          // purchases items shape: sellerEmail, status
          Array.isArray(purchases) && purchases.forEach((p: any) => {
            const seller = (p.sellerEmail || p.seller || '').toLowerCase();
            const status = (p.status || '').toString().toLowerCase();
            // treat anything not completed/finished as unfinished (pending, processing, etc.)
            if (seller && status !== 'completed' && status !== 'delivered' && status !== 'success') {
              unfinishedMap.set(seller, (unfinishedMap.get(seller) || 0) + 1);
            }
          });

          const reportsMap = new Map<string, number>();
          Array.isArray(reports) && reports.forEach((r: any) => {
            const seller = (r.sellerEmail || '').toLowerCase();
            if (seller) reportsMap.set(seller, (reportsMap.get(seller) || 0) + 1);
          });

          const enriched = data.map((req: any) => ({
            ...req,
            sellerReportsCount: reportsMap.get((req.email || '').toLowerCase()) || 0,
            hasUnfinishedOrders: (unfinishedMap.get((req.email || '').toLowerCase()) || 0) > 0,
          }));

          setRequests(enriched);
        } catch (innerErr) {
          console.error('Error fetching purchases/reports:', innerErr);
          setRequests(data);
        }
      }
    } catch (error) {
      ...
      <TableContainer component={Paper} elevation={2} sx={{ borderRadius: 2, background: "linear-gradient(145deg, #ffffff, #f8fafc)", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
        {loading ? (
          ...
          <Table>
            <TableHead sx={{ bgcolor: "#F9FAFB" }}>
              <TableRow>
                {["Date", "User", "Method", "Amount", "Status", "Reports", "Block", "Action"].map((h) => (
                  <TableCell key={h} sx={{ fontWeight: 600, color: "#6B7280", fontSize: "12px", textTransform: "uppercase", py: 2.5 }}>
                    {h}
                  </TableCell>
                ))}
            ...
            <TableBody>
              {paginatedRequests.map((r) => (
                <TableRow key={r._id} hover sx={{ "&:hover": { background: "rgba(51,172,111,0.03)" } }}>
                  ...
                  <TableCell sx={{ textAlign: 'center' }}>{r.sellerReportsCount || 0}</TableCell>
                  <TableCell>
                    {r.hasUnfinishedOrders ? (
                      <Typography component="span" sx={{ fontSize: '12px', fontWeight: 700, color: '#B45309', background: '#FFFAEB', px: 1.5, py: 0.5, borderRadius: 1 }}>Blocked</Typography>
                    ) : (
                      <Typography component="span" sx={{ fontSize: '12px', color: '#6B7280' }}>—</Typography>
                    )}
                  </TableCell>
                  <TableCell>
                    <Button size="small" onClick={() => openModal(r)} sx={{ textTransform: "none", bgcolor: "transparent", color: "#6B21B6", borderRadius: 1, border: "1px solid rgba(107,33,182,0.08)", px: 2, py: 0.5 }} disabled={r.hasUnfinishedOrders}>Review</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
