...
const DashboardSeller: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [prodRes, purchaseRes, ratingsRes, referralRes] = await Promise.all([
          axios.get<any[]>("http://72.244.153.24:3200/product/all-sells"),
          axios.get<any[]>("http://72.244.153.24:3200/purchase/getall", { params: { email: user.email, role: 'seller' } }),
          axios.get<any>(`http://72.244.153.24:3200/rating/seller/${user.email}`),
          axios.get<any>("http://72.244.153.24:3200/referral/stats", { params: { email: user.email } }),
        ]);
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const res = await axios.get("http://72.244.153.24:3200/purchase/report/getall");
