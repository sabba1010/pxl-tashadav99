...
const SellForm: React.FC = () => {
  ...
  const nextStep = () => {
    ...
    if (step === 0 && validateStep0()) {
      ...
      // On step 1, when user clicks "Submit Listing", they should have saved accounts
      if (loadingCredit) {
      ...
      // Call handleSubmit which will submit all saved accounts
      handleSubmit();
    }
  };
  ...
  const handleSubmit = async () => {
    // Check if there are saved accounts to submit
    if (savedAccounts.length === 0) {
      toast.error("Please add at least one account before submitting.");
      return;
    }
    ...
    try {
      // Format data for backend: remove the 'id' and 'savedAt' fields, keep required fields
      const productsToSubmit = savedAccounts.map((account) => ({
      ...
      if (response.status === 201 || response.status === 200) {
        const responseData = response.data as { insertedCount: number; message: string };
        toast.success(
          `${responseData.insertedCount} account(s) listed successfully!`
        );
        ...
        // Update local credit count (backend deducts only 1 credit regardless of count)
