...
const SellForm: React.FC = () => {
    ...
    const fetchPlatforms = async () => {
      try {
        const response = await axios.get<{ data: Platform[] }>(
          "http://72.244.153.24:3200/icon-data"
        );
    ...
    const fetchSalesCredit = async () => {
      ...
      try {
        const response = await axios.get<{ salesCredit: number }>(
          `http://72.244.153.24:3200/product/credit?email=${encodeURIComponent(
            user.email
          )}`
        );
  ...
  const validateStep0 = () => {
    ...
    if (!formData.deliveryType) {
      newErrors.deliveryType = "Please select a release option";
    }
    if (formData.deliveryType === "manual" && !formData.deliveryTime?.trim()) {
      newErrors.deliveryTime = "Please select or enter expected delivery time";
    }
    ...
    if (savedAccounts.length > 0) {
      ...
      try {
        const response = await axios.post<SellResponse>(
          "http://72.244.153.24:3200/product/sell",
                ...
                <RadioGroup
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({
                      ...formData,
                      deliveryType: value,
                      deliveryTime: value === "automated" ? null : formData.deliveryTime,
                    });
                    if (value === "automated") {
