...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const newSocket = io("https://acctempire.com/api"); // Adjust URL for production
        setSocket(newSocket);
    ...
    const fetchInitialCounts = async (uid: string) => {
        try {
            const res = await axios.get<Record<string, number>>(`https://acctempire.com/api/chat/unread/counts/${uid}`);
    ...
    const markOrderRead = (orderId: string) => {
        ...
        if (socket && userId) {
            // Also call API
            axios.post(`https://acctempire.com/api/chat/mark-read`, { userId, orderId })
