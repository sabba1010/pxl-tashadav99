...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const newSocket = io("http://72.244.153.24:3200"); // Adjust URL for production
        setSocket(newSocket);
    ...
    const fetchInitialCounts = async (uid: string) => {
        try {
            const res = await axios.get<Record<string, number>>(`http://72.244.153.24:3200/chat/unread/counts/${uid}`);
    ...
    const markOrderRead = (orderId: string) => {
        ...
        if (socket && userId) {
            // Also call API
            axios.post(`http://72.244.153.24:3200/chat/mark-read`, { userId, orderId })
